...
export default function HotelsPage() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-6 pt-8 pb-16">
      <div className="mb-6">
        <h1 className="text-3xl font-extrabold">🏨 Bakıda Top Otellər</h1>
        <p className="text-muted-foreground text-sm mt-1">Top {BAKU_HOTELS.length} otel — Luxury-dən Budget-ə</p>
      </div>

      <input
        value={search}
        onChange={e => setSearch(e.target.value)}
        placeholder="Otel axtar..."
...
function HotelCard({ hotel }: { hotel: BakuHotel }) {
  ...
  const tierLabels: Record<string, string> = { luxury: "💎 Luxury", mid: "⭐ Orta", budget: "💰 Büdcə", economy: "💸 Ucuz" };
