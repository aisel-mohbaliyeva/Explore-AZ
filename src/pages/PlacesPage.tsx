...
export default function PlacesPage() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-6 pt-8 pb-16">
      <div className="mb-6">
        <h1 className="text-3xl font-extrabold">{t("baku.places.title")}</h1>
        <p className="text-muted-foreground text-sm mt-1">{t("baku.places.topCount", { count: BAKU_PLACES.length })}</p>
      </div>
      ...
      <input
        ...
        placeholder={t("baku.places.search")}
      ...
      <div className="flex gap-2 flex-wrap mb-6">
        {Object.entries(PLACE_CATEGORIES).map(([key, label]) => (
          ...
          >
            {key === "all" ? t("category.all") : t(`place.category.${key}`)}
          </button>
        ))}
      </div>
