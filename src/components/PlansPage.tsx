...
const BUDGET_INFO = {
  luxury: { labelKey: "budget.luxury", icon: "💎", daily: "250-400 AZN", color: "#E8A838" },
  medium: { labelKey: "budget.medium", icon: "⭐", daily: "100-180 AZN", color: "#52C77C" },
  budget: { labelKey: "budget.budget", icon: "💰", daily: "40-80 AZN", color: "#4A9FFF" },
};
    ...
    <div className="max-w-[1100px] mx-auto px-4 sm:px-6 pt-6 sm:pt-8 pb-12 sm:pb-16">
        ...
        {TRIP_PLANS.map((plan) => (
          ...
          >
            <div className="flex items-center gap-3 mb-3 sm:mb-4">
              ...
              <div>
                <div className="font-bold text-lg sm:text-xl">{t("plans.daysPlan", { days: plan.days })}</div>
                <div className="text-xs sm:text-sm text-muted-foreground">{t("plans.fullRoute")}</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] sm:text-xs text-muted-foreground">{t("plans.fullPlan")}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-border p-4 sm:p-5 mb-6 sm:mb-8" style={{ background: "#111827" }}>
        <div className="flex items-center gap-2 mb-3 sm:mb-4">
          ...
          <span className="font-bold text-sm sm:text-base">{t("plans.budget")}</span>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="px-4 sm:px-6 pt-4 sm:pt-5 flex items-start justify-between">
              <div>
                <h2 className="text-lg sm:text-2xl font-extrabold tracking-tight flex items-center gap-2">
                  {t("plans.azerbaijanPlanTitle", { days: selectedPlan.days })}
                </h2>
                <div className="text-xs sm:text-sm text-muted-foreground mt-1">
                  {t("plans.routeIncluded")}
                </div>
              </div>
              <button
              ...
              {selectedPlan.dailyPlans[currentDay] && (
                <div className="animate-fade-in">
                  <div className="text-base sm:text-lg font-bold mb-3 sm:mb-4 flex items-center gap-2">
                    {t("plans.dayTitle", {
                      day: selectedPlan.dailyPlans[currentDay].day,
                      title: selectedPlan.dailyPlans[currentDay].title,
                    })}
                  </div>
                  ...
                  <div className="mb-4 sm:mb-5">
                    <div className="flex items-center gap-2 mb-2.5 sm:mb-3">
                      ...
                      <span className="font-semibold text-xs sm:text-sm">{t("plans.places")}</span>
                    </div>
                    <div
                  ...
                  {selectedPlan.dailyPlans[currentDay].restaurants.length > 0 && (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2.5 sm:mb-3">
                        ...
                        <span className="font-semibold text-xs sm:text-sm">{t("plans.restaurants")}</span>
                      </div>
                          ...
                          return (
                            <div key={level} className="flex items-start gap-2">
                              <span className="text-xs sm:text-sm shrink-0" style={{ color: BUDGET_INFO[level].color }}>
                                {BUDGET_INFO[level].icon} {t(BUDGET_INFO[level].labelKey)}:
                              </span>
                  ...
                  {selectedPlan.dailyPlans[currentDay].hotels.length > 0 && (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2.5 sm:mb-3">
                        ...
                        <span className="font-semibold text-xs sm:text-sm">{t("plans.hotels")}</span>
                      </div>
                      <div className="space-y-1.5 sm:space-y-2">
                        {(["luxury", "medium", "budget"] as const).map((level) => {
                  ...
                  {currentDay > 0 && (
                    ...
                    >
                      ...
                      <div className="text-lg sm:text-xl font-extrabold mb-2">{t("plans.locked")}</div>
                      <p className="text-xs sm:text-sm text-muted-foreground text-center max-w-[280px] sm:max-w-[300px] mb-4 sm:mb-5 px-4">
                        {t("plans.lockedDesc")}
                      </p>
                      <button
                        onClick={() => {
                          alert(t("plans.purchaseSuccess", { days: selectedPlan.days }));
                      ...
                      >
                        ...
                        {t("plans.unlock")} · {selectedPlan.price}
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-border flex items-center justify-between">
              <div>
                <div className="text-xs sm:text-sm text-muted-foreground">{t("plans.priceLabel")}</div>
              ...
              >
                ...
                {t("plans.buyPlan")} · {selectedPlan.price}
              </button>
            </div>
          </div>
        </div>
